<template>
    <el-drawer v-model="localVisible" title="编辑试题" @close="handleClose" size="50%">
        <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
            <el-form-item label="题目正文" prop="question">
                <el-input v-model="form.question" placeholder="请输入题目正文"></el-input>
            </el-form-item>
            <el-form-item label="题目类型" prop="questionType">
                <el-select v-model="form.question_type" placeholder="请选择题目类型" disabled>
...
const props = defineProps(['visible', 'questionData'])
...
function handleSubmit() {
    formRef.value.validate(async (valid: boolean) => {
        if (valid) {
            const response: any = await editQuestion(form.value)
